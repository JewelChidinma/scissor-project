...
async function authenticateUser(req, res){
    ...
    if(!isMatch) return res.status(400).json({message: "Passwords do not match", data: null})

    let payload = {email: req.body.email, }
    let token = jwt.sign(payload, process.env.JWT_KEY, {expiresIn: '1d'})
    req.user = payload

    return res.status(200).json({message: "Logged in successfully", data: {...user, token: token}})
}
